# 🏗️ Laravel Multi-Tenant API Starter

A complete starter kit to quickly bootstrap a multi-tenant Laravel API, architected with best practices and production-ready.
...
This project provides a solid foundation for building multi-tenant SaaS applications with Laravel. It uses the **"database per tenant"** approach for complete data isolation, particularly suitable for applications requiring strict compliance (HIPAA, GDPR).
...
Route::middleware(['auth:sanctum', 'permission.tenant:customers.create'])->group(function () {
    Route::post('/customers', [ProductController::class, 'store']);
});
```
...
- [ ] Add more supported databases
- [ ] Add more supported authentication methods
- [ ] Do Tenant identification strategy configurable
- [ ] Add Pest testing
---
